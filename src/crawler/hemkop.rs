...
pub struct HemkopCrawler;

#[async_trait]
impl StoreCrawler for HemkopCrawler {
    async fn crawl_price(&self, task: CrawlTask) -> Result<CrawlResult> {
        let url = format!("https://www.hemkop.se/axfood/rest/p/{}", task.fetch_id);
...
mod tests {
    ...
    async fn test_crawl_price() {
        let crawler = HemkopCrawler;
        let task = CrawlTask {
            id: 1,
            butik_id: None,
            butik_name: "".to_string(),
            fetch_id: "100748746_KG".to_string(),
        };
